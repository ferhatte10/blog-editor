...
ArticleController.create = async (req, res) => {
  ...
  const author_id = req.auth.user_info.id;
...
ArticleController.incrementViewCount = async (req, res) => {
  const id = parseInt(req.params.id);
  ...
  try {
    ...
    // Step 2: Increment the view_count for the article
    await existingArticle.increment('view_count');
...
ArticleController.update = async (req, res) => {
  ...
  try {
    ...
    // Step 3: Fetch the updated article
